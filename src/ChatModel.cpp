...
ChatModel::ChatModel(Locale *locale, StorageManager *storageManager, QObject *parent)
...
QVariant ChatModel::data(const QModelIndex &index, int role) const
{
    ...
    switch (role)
    {
        ...
        case TitleRole:
            return QString::fromStdString(chat->title_) /*Utils::getChatTitle(chatId, m_storageManager, m_locale, true)*/;
        case PhotoRole: {
...
void ChatModel::populate()
{
    ...
    for (const auto &id : chatIds)
    {
        ...
        {
            if (comparator(position->list_, Utils::toChatList(m_chatList)))
            {
...
void ChatModel::handleChatItem(qint64 chatId)
{
    if (auto it = std::ranges::find(m_chatIds, chatId); it != m_chatIds.end())
    {
        ...
        qDebug() << QString::fromStdString(m_storageManager->getChat(chatId)->title_);
    }
}
...
void ChatModel::loadChats()
{
    ...
    m_client->send(std::move(request), [this](auto &&value) {
        if (value->get_id() == td::td_api::error::ID)
        {
            if (auto error = td::move_tl_object_as<td::td_api::error>(value); error->code_ == 404)
            {
                m_loading = false;
                m_loadingTimer->stop();
